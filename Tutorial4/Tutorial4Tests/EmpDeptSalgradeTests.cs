...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var deptNosInChicago = depts.Where(s => s.Loc == "CHICAGO").Select(d => d.DeptNo);
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e=> grades,
            (e,s)=> new {e.EName,e.Sal,s.Grade,s.Losal,s.Hisal}).Where(r=> r.Sal >= r.Losal && r.Sal <= r.Hisal)
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var deptAvg = emps.GroupBy(d=>d.DeptNo)
            .ToDictionary(g=>g.Key, g=>g.Average(e=>e.Sal));

        var result = emps.Where(e=> e.Sal > deptAvg[e.DeptNo])
